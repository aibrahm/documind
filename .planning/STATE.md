...
## Active session
2026-04-07 — Phase 08 (deferred cleanup) shipped after the one-shot run. Single sweep through every deferred enhancement logged across phases 03 → 07. 9 atomic commits closed: SHA256-based exact duplicate detection (proper 31% bug fix), workspace-level PDF viewer context, per-conversation rename/delete restored in ProjectSidebar, multi-project suggestions (top 3) + create-new-from-upload flow, localStorage persistence (sidebar collapse + project expansion), logger module + audit row per chat turn, 4 fail-loud fixes (intelligence-router parse, memory swallow, Cohere rerank, Tavily already-throwing), financial_model currency field, fetch_url PDF support via pdf-parse, React error boundaries (root + workspace), project-doc dedupe by document_id, parallel pinned-entity search. Still genuinely deferred (architectural / framework): test suite, console.error → logger migration (chokepoint exists), token-usage cost tracking, references viewer page, chat-turn.ts split, Next.js 16 notFound() 200 quirk. Roadmap: ~~02~~ → ~~03~~ → ~~03.5~~ → ~~04~~ → ~~04.5~~ → ~~05~~ → ~~06~~ → ~~07~~ → ~~08 cleanup~~ → 09+ (save-as-artifact, casual mode tools, disambiguation, tier 2 tools).
