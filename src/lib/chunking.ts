...
function chunkSection(
  ...
  const flush = (withBridgeFromPrevious: boolean) => {
    ...
    const bridge = withBridgeFromPrevious && chunks.length > 0
      ? getOverlapSuffix(chunks[chunks.length - 1].content, INTRA_SECTION_OVERLAP_CHARS)
      : "";
  ...
  for (const sentence of sentences) {
    if (currentChunk.length + sentence.length > MAX_CHUNK_CHARS && currentChunk) {
      flush(true);
    }
...
function mergeTinyTails(chunks: DocumentChunk[]): DocumentChunk[] {
  ...
  for (const c of chunks) {
    ...
    const wouldStillFit =
      last && last.content.length + c.content.length + 1 <= MAX_CHUNK_CHARS * 1.2;
    ...
    if (tooSmall && samePage && wouldStillFit && last) {
      ...
      last.metadata = {
        ...last.metadata,
        originalLength: last.metadata.originalLength + c.metadata.originalLength,
      };
      // Adopt section/clause title only if previous didn't have one
      if (!last.sectionTitle && c.sectionTitle) last.sectionTitle = c.sectionTitle;
      if (!last.clauseNumber && c.clauseNumber) last.clauseNumber = c.clauseNumber;
