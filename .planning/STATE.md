...
## Active session
2026-04-07 — ONE-SHOT THROUGH PHASE 07 COMPLETE. Phases shipped this session: 04.5 (Link & Create UI affordances), 05 (project-scoped chat backend with additive retrieval), 07 (librarian project intelligence + 31% bug fix), 06 (expandable deal-room view in Negotiations tab). The full project-centric vision is now live end-to-end: drop a PDF → librarian proposes classification + project + dedup, one-click confirms all three → open project workspace → chat is automatically scoped (project docs first, PUBLIC/DOCTRINE always available, other projects' PRIVATE excluded) → deep mode fires extract_key_terms → negotiation auto-populated → compare_deals lays out scenarios → financial_model returns exact NPVs → click a negotiation in the workspace → expanded deal-room view shows the timeline + key facts. Roadmap: ~~02~~ → ~~03~~ → ~~03.5~~ → ~~04~~ → ~~04.5~~ → ~~05~~ → ~~06~~ → ~~07~~ → 08+ (model badge, save-as-artifact, casual mode tools, disambiguation, tier 2 tools).
